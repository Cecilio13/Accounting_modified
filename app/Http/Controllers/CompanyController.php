...
class CompanyController extends Controller
{
    ...
    public function update_company(Request $request)
    {
        ...
        return redirect('accountsandsettings');
    }
    ...
    public function getSessionTimeout(Request $request){
        $advance = Advance::first();
        return $advance->advance_inactive_time; 
    }
    ...
    public function sync_data_v_two(Request $request){
        ...
        try {
            ...
            $db = DB::connection('mysql3')->select($query, ['acc_system']);
            if(empty($db)){
